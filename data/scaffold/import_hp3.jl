...
hp3_assoc, hp3_hosts, hp3_viruses = [CSV.read(joinpath(hp3_path, "$(hp3f).csv"), missingstrings=["NA", "Unassigned"]) for hp3f in hp3_files]
...
for (idx, host_row) in enumerate(eachrow(hp3_hosts))
    ...
    if !(host_row.hHostNameFinal in hp3_entities_host.name)
        push!(hp3_entities_host, (entity_hash, "HP3", host_row.hHostNameFinal, match_gbif_hash))
    end
end
...
for (i,row) in enumerate(eachrow(hp3_assoc))
    ...
    push!(hp3_associations, (
        association_id, host_id, virus_id,
        source, index, method
    ))
end
